		...
		$.ajax({
			...
			success : function(result) {
				...
				if (result == "SUCCESS") {
					...
					htmls += '<button type="button" class=" btn-light px-5" data-toggle="modal" data-target="#return_'+member_id+'">되돌리기</button>'
					tdHtmls += '<div>탈퇴</div>'
		...
		$.ajax({
			...
			success : function(result) {
				...
				if (result == "SUCCESS") {
					...
					htmls += '<button type="button" class=" btn-light px-5" data-toggle="modal" data-target="#withdrawal_'+member_id+'">탈퇴</button>'
					tdHtmls += '<div>회원</div>'
...
<%--		총 회원 수 : ${total} &nbsp;&nbsp; 탈퇴한 회원 수 : ${withdrawal}
		<br/> --%>
			...
			<form class="search-bar" action="${pageContext.request.contextPath}/search/member" method="get">			
				<input class="col-2" type="text" class="form-control" name="keyword" placeholder="회원 검색" value="${keyword}"/>
				<button type="submit" class=" btn-light px-5"><i class="icon-magnifier"></i></button>
			</form>
			</div>
				...
				<div class="row mt-3">
					<div class="col-lg-12">
						<div class="card">
							<div class="card-body">
              <!-- <h1 class="card-title"><a href="/admin">회원 관리</a> </h1> -->
